...
NODE_REGISTRY: Dict[str, NodeTypeSpec] = {
    ...
    "sql_query": NodeTypeSpec(
        node_type="sql_query",
        display_name="SQL Query",
        category="Transform",
        color="#1d3461",
        description="Run a SQL query on the input DataFrame using Polars SQLContext",
        input_ports=["data_in"],
        params=[
            NodeParamSpec(
                "sql",
                ...
                "string",
                required=True,
                tooltip="SQL query to execute. Reference the input table using {{table_name}}",
            ),
            NodeParamSpec(
                "table_name",
                "Table Name",
                "string",
                "data",
                tooltip="Name to register the input DataFrame as in SQL context",
            ),
        ],
    ),
    "sqlite_reader": NodeTypeSpec(
        node_type="sqlite_reader",
        display_name="SQLite Reader",
        category="Source",
        color="#1b4332",
        description="Load data from a SQLite database query",
        input_ports=[],
        params=[
            NodeParamSpec(
                "file_path",
            ...
            NodeParamSpec(
                ...
                "string",
                required=True,
                tooltip="SQL query to execute (e.g. SELECT * FROM table)",
            ),
        ],
    ),
    "duckdb_reader": NodeTypeSpec(
        node_type="duckdb_reader",
        display_name="DuckDB Reader",
        category="Source",
        color="#1b4332",
        description="Load data using DuckDB (query files, databases, or inline SQL)",
        input_ports=[],
        params=[
            NodeParamSpec(
                "query",
                ...
                "string",
                required=True,
                tooltip="DuckDB SQL query (e.g. SELECT * FROM 'data.csv')",
            ),
            NodeParamSpec(
                "read_only",
                "Read Only",
                "bool",
                False,
                tooltip="Open DuckDB in read-only mode (disable for in-memory queries)",
            ),
        ],
    ),
    "postgres_reader": NodeTypeSpec(
        node_type="postgres_reader",
        display_name="PostgreSQL Reader",
        category="Source",
        color="#1b4332",
        description="Load data from a PostgreSQL database query",
        input_ports=[],
        params=[
            NodeParamSpec(
                "connection_string",
                ...
                "string",
                required=True,
                tooltip="PostgreSQL connection URI (e.g. postgresql://user:pass@host/db)",
            ),
            NodeParamSpec(
                "query",
                "Query",
